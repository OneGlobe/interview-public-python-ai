...
def list_messages(conversation_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def get_message(conversation_id: int, message_id: int, db: Session = Depends(get_db)):
